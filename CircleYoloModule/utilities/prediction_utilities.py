...
from .map_utilities import non_max_suppression

def example_prediction(model, test_dataset, loss_yolo_fn = None, C=1, S=7, B=1, prob_threshold = 0.5, iou_threshold = 0.51, device="cpu"):
  """
  ...
  if loss_yolo_fn:
    ...
    loss_fn = loss_yolo_fn(C=C, S=S, B=B, lambda_noobj=0.5, lambda_coord=5)
    test_loss = loss_fn(out_label, labels.unsqueeze(dim=0));

  # Reshape the model output to the desired format
  out_label = out_label.reshape(S, S, -1)
  ...
  apples = out_labels_apple(labels = out_label, threshold = prob_threshold, C=C, S=S, B=B)
